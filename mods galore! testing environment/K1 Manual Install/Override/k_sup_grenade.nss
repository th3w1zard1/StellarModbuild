...
void main()
{
    ...
    if(nSpell == 87) //Fragmentation
    {
        ...
        nVFX = 3003;
        nDamage = 20 + nFeat + nDemo/2 + nDemo;
        nDC = 15;

        oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 5.0, GetSpellTargetLocation());
    ...
    else if(nSpell == 89)///Thermal Detonator
    {
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(1044),GetSpellTargetLocation());
        nVFX = 3005;
        nDamage = 60 + nFeat + nDemo;
        nDC = 15;
        effect ePush = EffectForcePushTargeted(GetSpellTargetLocation());

        oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 6.0, GetSpellTargetLocation());
        while(GetIsObjectValid(oTarget))
        {
            if(!GetIsNeutral(oTarget))
            {
            ...
            oTarget = GetNextObjectInShape(SHAPE_SPHERE, 6.0, GetSpellTargetLocation());
        }
    }
            ...
            if(!GetIsNeutral(oTarget))
            {
                ...
                if(!ReflexSave(oTarget, nDCCheck, SAVING_THROW_TYPE_COLD))
                {
                    if(!GetIsLinkImmune(oTarget, eDex) || GetRacialType(oTarget) == RACIAL_TYPE_DROID)
                    {
                      ...
                      ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectMovementSpeedDecrease(25), oTarget);

                     }
                    ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(nDamage, DAMAGE_TYPE_COLD), oTarget); 
                    ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectMovementSpeedDecrease(10), oTarget);
                }
                else
                {
